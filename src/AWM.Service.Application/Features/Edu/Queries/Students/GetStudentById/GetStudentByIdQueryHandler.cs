    ...
    public async Task<Result<StudentDto>> Handle(GetStudentByIdQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (student is null || student.IsDeleted)
                return Result.Failure<StudentDto>(new Error(DomainErrors.Edu.Student.NotFound, $"Student with ID {request.StudentId} not found."));
        ...
        {
            return Result.Failure<StudentDto>(new Error(DomainErrors.General.InternalError, $"An error occurred: {ex.Message}"));
        }
    }
}
