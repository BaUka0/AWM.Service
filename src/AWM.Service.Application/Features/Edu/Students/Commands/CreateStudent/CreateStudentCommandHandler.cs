    ...
    public async Task<Result<int>> Handle(CreateStudentCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (existingStudent is not null && !existingStudent.IsDeleted)
                return Result.Failure<int>(new Error("409", $"Student profile for user {request.UserId} already exists."));
        ...
        {
            return Result.Failure<int>(new Error("500", $"An error occurred while creating the Student: {ex.Message}"));
        }
    }
}
