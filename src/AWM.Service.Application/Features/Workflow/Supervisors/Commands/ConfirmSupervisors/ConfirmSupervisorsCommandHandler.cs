...
public sealed class ConfirmSupervisorsCommandHandler : IRequestHandler<ConfirmSupervisorsCommand, Result>
{
    ...
    public ConfirmSupervisorsCommandHandler(
    ...
    public async Task<Result> Handle(ConfirmSupervisorsCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result.Failure(new Error("Supervisors.NoSupervisorsToConfirm", "No supervisor assignments found to confirm."));
        }
        ...
        var supervisorUserIds = new List<int>();
        ...
        foreach (var assignment in filteredAssignments)
        {
            ...
            supervisorUserIds.Add(assignment.UserId);
        }

        // Raise domain event on the first assignment
        if (filteredAssignments.Count > 0)
        {
            filteredAssignments[0].RaiseSupervisorsApprovedEvent(
                request.OrgUnitId,
                request.SemesterId,
