...
public sealed class TransitionStateCommandHandler : IRequestHandler<TransitionStateCommand, Result>
{
    ...
    public async Task<Result> Handle(TransitionStateCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                "direction" => await TransitionDirectionAsync(request, userId.Value, cancellationToken),
                "studentwork" or "work" => await TransitionStudentWorkAsync(request, userId.Value, cancellationToken),
                _ => Result.Failure(new Error(DomainErrors.Workflow.GenericError, $"Unknown entity type: {request.EntityType}"))
            };
        }
        catch (InvalidOperationException opEx)
        {
            return Result.Failure(new Error(DomainErrors.Workflow.InvalidTransition, opEx.Message));
        }
        catch (Exception ex)
        {
            return Result.Failure(new Error(DomainErrors.General.InternalError, $"An error occurred during state transition: {ex.Message}"));
        }
    }
    ...
    private async Task<Result> TransitionDirectionAsync(TransitionStateCommand request, int userId, CancellationToken cancellationToken)
    {
        ...
        if (direction is null || direction.IsDeleted)
            return Result.Failure(new Error(DomainErrors.Workflow.EntityNotFound, $"Direction with ID {request.EntityId} not found."));
        ...
        if (!canTransition)
            return Result.Failure(new Error(DomainErrors.Workflow.InvalidTransition, $"Transition from current state to state {request.TargetStateId} is not allowed."));
        ...
        if (targetState is null)
            return Result.Failure(new Error(DomainErrors.Workflow.StateNotFound, $"Target state with ID {request.TargetStateId} not found."));
        ...
        switch (targetState.SystemName)
        {
            ...
            case Domain.Wf.Entities.DirectionStates.RequiresRevision:
                ...
                break;
            default:
                return Result.Failure(new Error(DomainErrors.Workflow.InvalidTransition, $"Unsupported target state: {targetState.SystemName}"));
        }
    ...
    private async Task<Result> TransitionStudentWorkAsync(TransitionStateCommand request, int userId, CancellationToken cancellationToken)
    {
        ...
        if (work is null || work.IsDeleted)
            return Result.Failure(new Error(DomainErrors.Workflow.EntityNotFound, $"StudentWork with ID {request.EntityId} not found."));
