...
public sealed class GetAvailableTransitionsQueryHandler : IRequestHandler<GetAvailableTransitionsQuery, Result<IReadOnlyList<TransitionDto>>>
{
    ...
    public async Task<Result<IReadOnlyList<TransitionDto>>> Handle(GetAvailableTransitionsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                WorkflowEntityTypes.Direction => await GetDirectionStateAsync(request.EntityId, cancellationToken),
                WorkflowEntityTypes.StudentWork or WorkflowEntityTypes.Work => await GetStudentWorkStateAsync(request.EntityId, cancellationToken),
                _ => -1
            };

            if (currentStateId <= 0)
                return Result.Failure<IReadOnlyList<TransitionDto>>(new Error("404", $"Entity '{request.EntityType}' with ID {request.EntityId} not found."));
        ...
        {
            return Result.Failure<IReadOnlyList<TransitionDto>>(new Error("500", $"An error occurred: {ex.Message}"));
        }
    }
