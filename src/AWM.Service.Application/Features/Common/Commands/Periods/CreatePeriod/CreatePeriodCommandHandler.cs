    ...
    public async Task<Result<int>> Handle(CreatePeriodCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (academicYear is null)
                return Result.Failure<int>(new Error(DomainErrors.Common.AcademicYear.NotFound, $"Academic year with ID {request.AcademicYearId} not found."));
            ...
            if (overlapping)
                return Result.Failure<int>(new Error(DomainErrors.Common.Period.OverlappingPeriod, "An overlapping period for this workflow stage already exists."));
        ...
        {
            return Result.Failure<int>(new Error(DomainErrors.Common.Period.GenericError, argEx.Message));
        }
        catch (Exception ex)
        {
            return Result.Failure<int>(new Error(DomainErrors.General.InternalError, $"An error occurred while creating the Period: {ex.Message}"));
        }
    }
}
