namespace AWM.Service.Application.Features.Common.Commands.Periods.UpdatePeriod;
    ...
    public async Task<Result> Handle(UpdatePeriodCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (period is null || period.IsDeleted)
                return Result.Failure(new Error(DomainErrors.Common.Period.NotFound, $"Period with ID {request.PeriodId} not found."));
        ...
        {
            return Result.Failure(new Error(DomainErrors.Common.Period.GenericError, argEx.Message));
        }
        catch (Exception ex)
        {
            return Result.Failure(new Error(DomainErrors.General.InternalError, $"An error occurred while updating the Period: {ex.Message}"));
        }
    }
}
